...
// 50% Ar + 50% CH4
 ...
 density = 0.9*mg/cm3 ; //currently unknown
  G4Material *Ar_50CH4_50 = new G4Material(name="ArCH4_5050", density, ncomponents=2, kStateGas,300*kelvin,1.*atmosphere);
  Ar_50CH4_50->AddElement(elAr, fractionmass = 0.5); //fraction mass has to be changed
  Ar_50CH4_50->AddMaterial(CH4, fractionmass = 0.5 ); // fraction mass has to  be changed
  Ar_50CH4_50->GetIonisation()->SetMeanEnergyPerIonPair(fInput->GetGasFlag().WmeanAr_50CH4_50 *eV);
  ...
  if(detectorFlag == 4) detectorGas = Ar_50CH4_50;
  ...
  if(fInput->GetPitch() == 800){

  MeshBox   = new G4Box("MeshBox", 5*cm, 5*cm,0.0001*cm);
  ellipse  = new G4EllipticalTube("ellipse",0.3*cm,0.54*cm,0.0001*cm);
  subtract = new G4SubtractionSolid("subtract", MeshBox, ellipse, 0, G4ThreeVector(0.,0.,0.));
  
  ellipseLog = new G4LogicalVolume(ellipse, detectorGas, "ellipseLog", 0, 0, 0);
  new G4PVPlacement(0, G4ThreeVector(0.32*cm, 0.32*cm, -5.0001*cm), ellipseLog,"gas",mother,0,0,true);
  
  MeshLog   = new G4LogicalVolume(subtract, FR_4, "MeshLog", 0, 0, 0);
  new G4PVPlacement(0, G4ThreeVector(0.,0.,-5.0001*cm), MeshLog,"Mesh", mother, 0,0,true);
